...
def _load_full_table_rows_from_seed() -> tuple[list[dict[str, object]], dict[str, int], list[str]]:
    ...
    if traz_path.exists() and traz_path.stat().st_size > 0:
        try:
            raw_text = _detailv_decode(traz_path)
...
def _build_detailv_sales_report(comparativo_summary: dict[str, object]) -> dict[str, object]:
    ...
    def _aggregate(items: list[dict[str, object]], period_label: str) -> dict[str, object]:
        order = {"Facturas": 1, "Factura exenta": 2, "N. de Debitos": 3, "N. de Creditos": 4, "Boletas": 5}
    ...
    latest_label = latest_dt.strftime("%d/%m/%Y")
    base["branches"] = [{"key": "all", "label": "Todas"}]
    base["default_branch"] = "all"
...
def _build_proyecciones_dashboard() -> dict:
    """Proyecciones ejecutivas: producción, pedidos, ventas y cobertura."""

    # ── Producción (TRAZABILIDAD) ──────────────────────────────────────────
    full_rows, _, _ = _load_full_table_rows_from_seed()
    prod: dict[str, dict] = {}
    etapas_labels = ["Corte", "Taller", "T.Ext", "Limpiado", "Lavandería", "Terminación"]
    etapas_keys   = ["corte_1", "taller", "t_externo", "limpiado", "lavanderia", "terminacion"]

    for row in full_rows:
        temp = str(row.get("temporada") or "")
        if not temp.isdigit() or not (MIN_DISPLAY_COLLECTION <= int(temp) <= MAX_DISPLAY_COLLECTION):
            continue
        if temp not in prod:
            prod[temp] = {"programado": 0, "proceso": 0, "bodega": 0, "etapas": [0]*6}
        prod[temp]["programado"] += int(row.get("programa") or 0) + int(row.get("proceso") or 0) + int(row.get("bodega") or 0)
        prod[temp]["proceso"]    += int(row.get("proceso")   or 0)
        prod[temp]["bodega"]     += int(row.get("bodega")    or 0)
        for i, k in enumerate(etapas_keys):
            prod[temp]["etapas"][i] += int(row.get(k) or 0)

    prod_list = []
    for temp in sorted(prod.keys()):
        d = prod[temp]
        programado = d["programado"]
        pct = round(d["bodega"] / programado * 100) if programado else 0
        prod_list.append({
            "temporada":  temp,
            "programado": programado,
            "proceso":    d["proceso"],
            "bodega":     d["bodega"],
            "pct_bodega": pct,
            "etapas":     [{"label": etapas_labels[i], "qty": d["etapas"][i]} for i in range(6)],
        })

    # ── Pedidos (PEDIDOS.Txt) ──────────────────────────────────────────────
    pedidos_data: dict[str, dict] = {}
    top_saldo: list[dict] = []
    seed_pedidos = SEED_DIR / "PEDIDOS.Txt"
    if seed_pedidos.exists():
        for enc in ("utf-8-sig", "cp1252", "latin-1"):
            try:
                with open(seed_pedidos, encoding=enc, errors="strict", newline="") as fh:
                    ped_rows = list(csv.DictReader(fh, delimiter=";"))
                break
            except Exception:
                ped_rows = []
        articulo_saldo: dict[str, dict] = {}
        for r in ped_rows:
            cole = str(r.get("COLECCION") or "").strip()
            sol  = _to_int(r.get("SOLICITADO"))
            des  = _to_int(r.get("DESPACHADO"))
            sal  = _to_int(r.get("saldo"))
            art  = str(r.get("ARTICULO") or "").strip()
            desc = str(r.get("DESCRIPCION") or "").strip()
            if not cole or not sol:
                continue
            if cole not in pedidos_data:
                pedidos_data[cole] = {"solicitado": 0, "despachado": 0, "saldo": 0}
            pedidos_data[cole]["solicitado"] += sol
            pedidos_data[cole]["despachado"] += des
            pedidos_data[cole]["saldo"]      += sal
            if art:
                if art not in articulo_saldo:
                    articulo_saldo[art] = {"articulo": art, "descripcion": desc, "saldo": 0, "solicitado": 0}
                articulo_saldo[art]["saldo"]      += sal
                articulo_saldo[art]["solicitado"] += sol
        top_saldo = sorted(
            [v for v in articulo_saldo.values() if v["saldo"] > 0],
            key=lambda x: -x["saldo"]
        )[:10]

    pedidos_list = []
    for cole in sorted(pedidos_data.keys()):
        d = pedidos_data[cole]
        pct = round(d["despachado"] / d["solicitado"] * 100) if d["solicitado"] else 0
        pedidos_list.append({
            "coleccion":   cole,
    ...
    # ── Ventas: comparativo T43 vs T42 ────────────────────────────────────
    bi_ventas = BI_DIR / "VENTAS-TOD-2026.CSV"
    ventas_path = bi_ventas if bi_ventas.exists() and bi_ventas.stat().st_size > 0 else SEED_DIR / "VENTAS-TOD-2026.CSV"
    ventas_por_temp: dict[str, dict] = {}
    if ventas_path.exists():
        for enc in ("utf-8-sig", "cp1252", "latin-1"):
            try:
                with open(ventas_path, encoding=enc, errors="strict", newline="") as fh:
                    v_rows = list(csv.DictReader(fh, delimiter=";"))
                break
            except Exception:
                v_rows = []
        for r in v_rows:
            ...
            if not temp.isdigit():
                continue
            if temp not in ventas_por_temp:
                ventas_por_temp[temp] = {"total": 0, "prendas": 0, "clientes": set(), "docs": set()}
            ventas_por_temp[temp]["total"]   += _to_int(r.get("Total"))
            ventas_por_temp[temp]["prendas"] += _to_int(r.get("Cant"))
            ventas_por_temp[temp]["clientes"].add(str(r.get("Rut") or "").strip())
            ventas_por_temp[temp]["docs"].add(str(r.get("Numero") or "").strip())

    ventas_list = []
    for temp in sorted(ventas_por_temp.keys()):
        d = ventas_por_temp[temp]
        ventas_list.append({
            "temporada": temp,
            "total":     d["total"],
            "prendas":   d["prendas"],
    ...
    # Proyección T43: si T43 lleva X% del total de T42, proyectar cierre
    t42 = ventas_por_temp.get("42", {})
    t43 = ventas_por_temp.get("43", {})
    t42_total = t42.get("total", 0) if isinstance(t42, dict) else 0
    t43_total = t43.get("total", 0) if isinstance(t43, dict) else 0
    pct_t43_vs_t42 = round(t43_total / t42_total * 100) if t42_total else 0
    proyeccion_t43 = round(t43_total / (pct_t43_vs_t42 / 100)) if pct_t43_vs_t42 > 0 else 0

    # ── Cobertura: stock vs saldo pedidos ─────────────────────────────────
    inv_rows = query_inventory_stock_rows(DB_PATH)
    stock_total  = sum(int(r.get("stock") or 0) for r in inv_rows)
    saldo_total  = sum(d["saldo"] for d in pedidos_data.values())
    cobertura_pct = round(stock_total / saldo_total * 100) if saldo_total else 0
    ...
    return {
        ...
        "top_saldo":        top_saldo,
        "ventas":           ventas_list,
        "pct_t43_vs_t42":  pct_t43_vs_t42,
        "proyeccion_t43":  proyeccion_t43,
        "t43_total":       t43_total,
        "t42_total":       t42_total,
...
def index():
    ...
    return render_template(
        ...
        proyecciones=_build_proyecciones_dashboard(),
    )
